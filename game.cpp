...
Game::Game(QWidget* parent, int _x_tile_count, float _speed)
    : QOpenGLWidget(parent)
    , x_tile_count(_x_tile_count)
    , speed(_speed) {
    first = true;
    status = true;
...
void Game::initializeGL() {
    ...
    red_player = new Player(x_tile_count, y_tile_count, Direction::UP, QVector2D(1, 2), QVector2D(1, 1), QVector3D(1.0f, 0.258823f, 0.0f), QVector3D(1.0f, 0.741177f, 0.0f));
    blue_player = new Player(x_tile_count, y_tile_count, Direction::DOWN, QVector2D(x_tile_count, y_tile_count - 1), QVector2D(x_tile_count, y_tile_count), QVector3D(0.0f, 0.352941f, 1.0f), QVector3D(0.0f, 1.0f, 0.352941f));
    ...
    tile_texture = new QOpenGLTexture(QImage(":/img/tile.png"));
    ...
    head_texture = new QOpenGLTexture(QImage(":/img/tile.png"));
...
void Game::drawTile(QVector2D pos, QVector4D color, bool head) {
    core->glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
bool Game::removeFromEmptyTile(QVector2D val) {
...
void Game::updateGame() {
    ...
    if (step_timer >= 1.0f) {
        ...
        if (red_player->snake.front() == QVector2D(food->x(), food->y())) {
            QSound::play(":/audio/big_bite.wav");
            red_eat_flag = true;
